      ...
      <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="item in BusquedaCurso" :key="item">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div @click="$router.push({name: 'docente_cursos_show' , params: {id:item.id}})" class="text-indigo-600 hover:text-indigo-900">DIV</div>
              </td>
            </tr>
...
export default { 
   computed:{
       ...mapGetters('docente_storevuex',['getCursos']),
       BusquedaCurso(){
           return this.getCursos( this.s_buscar)
       },
       
   ...mapState('docente_storevuex',['isLoading'])
   },
   data(){
       return {
           s_buscar:''
 ...
 methods: {
   ...mapActions('docente_storevuex',['loadCursos'])
 },
 created(){
   this.loadCursos()
 },
